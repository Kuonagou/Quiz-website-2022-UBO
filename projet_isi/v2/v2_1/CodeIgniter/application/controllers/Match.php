...
public function afficher()
{
		...
		$this->form_validation->set_rules('pseudo', 'pseudo', 'trim|required|alpha_numeric', array('required'=>'Veuillez saisir un pseudo !'));
...
public function reponse()
{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$data['id_question']=$this->db_model->get_all_info_match_que($this->input->post('code'));
			foreach($id_question as $i){
				$i['rep']=$this->input->post('$i');
				if($this->db_model->get_true_rep($this->input->post($i['rep'])) != NULL){
					$score= $score + 1;
				}
				$total=$i['nbquestion'];
			}
