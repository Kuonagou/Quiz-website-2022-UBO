...
//	-new_match : crée un nouveau match désactiver avec la date du jour
//	-get_all_info_match_code : récupère toutes les infos d'un match dont le code est passé dans un paramètre
//	-get_count_match_nbque : compte le nombre de question d'un match
//	-get_count_match_idque : donne l'intitulé de toutes les questions d'un match dont le code est passé en paramètre
//	-get_all_info_match_donne : récupère toutes les info des tout les matchs
...
//	-reset : remet le match à zéro avec la date de debut à demain et la date de fin à null grace au code du match en paramètre 
//	-delete : supprime le match et c'est pseudo en appelant un procédure grace au code du match passé en paramètre
//	-activ : active ou désactive un quiz en fonction du paramètre passer en paramètre et du code du match
//	-score_match : calcul le score moyen des joueurs d'un match dont le code est passé en paramètre
//
...
//	-is_count : vérifie si un couple mdp pseudo appartient ou nom à la base 
//	-get_all_info_formateur : récupère toutes les info d'un formateur dont le pseudo est passé en paramètre
//	-get_all_info_admin : récupère toutes les info d'un formateur dont le pseudo est passé en paramètre
//	-change_password : modifie le mdp à l'aide des données fournises par le formulaire
//-------------------------------------------------------------
// A noter :
...
/* fonctions qui gère les matchs */
	...
	public function get_count_match_nbque($donnee){

	$query = $this->db->query("SELECT COUNT(QUE_ID) as nb from T_MATCH_MAT join T_QUESTION_QUE using(QUI_ID) where MAT_CODE='$donnee';");
	//test de bon execution requette
	echo $donnee;
	return $query->row();
	}
	public function get_count_match_idque($donnee){

	$query = $this->db->query("SELECT QUE_INTITULEQUESTION from T_MATCH_MAT join T_QUESTION_QUE  using(QUI_ID) where MAT_CODE='$donnee';");
	//test de bon execution requette
	echo $donnee;
	return $query->result_array();
	}
	...
	/*test de bon execution requette*/
	$query = $this->db->query("SELECT id_match('$code') as MAT_ID");
	return $query->row();
	}
	...
	$query1 = $this->db->query("CALL supp_match('$code');");
	/*test de bon execution requette*/ 
	...
	public function get_player_answer($code,$id)
        {
                $query = $this->db->query("SELECT REP_VRAI from T_REPONSE_REP join T_QUESTION_QUE using (QUE_ID) join T_MATCH_MAT using (QUI_ID) where MAT_CODE='$code' and REP_ID='$id';");
		/*test de bon execution requette*/
                return $query->row();
        }

   public function get_all_answer(){
   		$query = $this->db->get(TRUE,'QUE_ID');
   		return $query->result();
   }
